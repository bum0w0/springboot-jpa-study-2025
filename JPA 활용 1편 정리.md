...
#### BindingResult
- 입력값 검증 결과를 담는 객체로, @Valid 또는 @Validated 어노테이션과 함께 사용될 때 유효성 검사에서 발생한 오류를 담아줌
- 검증 실패 시에도 컨트롤러가 예외 없이 계속 실행되게 해주며, hasErrors()로 검증 결과를 체크해서 사용자에게 다시 입력 폼을 보여줄지 저장할지를 결정할 수 있음

#### Form 객체 vs 엔티티 객체 직접 사용
웹 애플리케이션에서 사용자의 입력을 처리할 때, Form 객체(요청 DTO)를 사용할지, 엔티티 객체(JPA Entity)를 바로 사용할지 비교 분석
#### Form 객체 (DTO, Command 객체 등)를 사용하는 경우

| 장점 | 설명 |
|------|------|
| 관심사 분리 | 입력 데이터 구조와 DB 저장 구조를 분리할 수 있음 |
| 보안 강화 | 엔티티에 민감한 필드가 포함된 경우, 의도치 않은 변경 방지 |
| 유효성 검증 용이 | `@Valid`, `@NotNull` 등의 검증을 Form 객체에만 적용 가능 |
| 유지보수 용이 | UI 요구사항이 변경되어도 엔티티는 영향 없음 |
| 명시적 변환 | 컨트롤러에서 DTO → 엔티티로 명시적 변환 처리 가능 (예: Mapper 사용) |
...
| 문제점 | 설명 |
|--------|------|
| 보안 이슈 | 클라이언트가 보내는 요청으로 엔티티의 민감한 필드를 변경할 수 있음 |
| 캡슐화 위반 | 외부에 직접 DB 구조를 노출하는 셈 |
| 유효성 검증 혼란 | 검증 로직을 엔티티에 둘 경우, 여러 계층에 의존 발생 |
| 변경에 취약 | UI 요구사항 변화가 DB 구조에 직접 영향을 줄 수 있음 |
| 테스트 어려움 | 엔티티는 연관 관계나 로직을 포함하고 있어 단위 테스트 어려움 |

#### 언제 Form 객체를 사용하는 게 좋은가?

- 사용자 입력과 DB 저장 구조가 다를 때
- 민감한 필드를 보호해야 할 때
